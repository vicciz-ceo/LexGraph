...
- G13 Authored-text fidelity (ADDED post-audit, 2026-07-26 — the gap that let entity corruption reach `review`): ordinary authored prose round-trips BYTE-EXACT through every write path, explicitly including ampersands and entity-shaped text in normal business/legal usage (`R&D spend`, `AT&T`, `Smith &Jones`, `&#8212`, `&#160a`, `&pound;500 &amp; rising`), section signs, curly quotes, em-dashes, and multi-paragraph text. Sanitization may remove markup; it may never insert, reorder, or destroy authored characters, and must never return an empty string for input that contains no markup.
...
- 2026-07-26T01:33:44Z — Manager: CORRECTION — sprint re-opened from `review`. Independent 4-lens audit (18 agents) found 2 severe defect families QA missed; manager reproduced ALL: entity/charref `;` insertion corrupting `R&D`/`AT&T` and DESTROYING documents containing `&#160a`-shaped refs (fail-closed after non-convergence, O(n^2)); RCDATA suppression swallowing prose after `<Title>`. Ruling R16 records manager-probed fix direction (shield `&` from parser; suppress content only for script/style). Planner-role agent to author RED tests, then Developer. Director informed that prior "at review" report was premature.
...
- 2026-07-26T02:20:00Z — QA (Sonnet, high), cycle 6, final verification post-audit: baseline 216/216 backend + 60/60 frontend confirmed clean, all 13 audit-RED green, no pins regressed. G13 adversarially re-probed both directions: sentinel-smuggling safe (no forged/lost `&`, NUL-stripping deliberate/documented), narrowed script/style-only suppression re-battled across 23 wrapper shapes (0 leaks). Found ONE new live finding: `_sanitize_once` double-processes an unclosed CDATA/RCDATA wrapper's leftover against what `parser.close()` already flushed — causes text duplication (1 trigger), document destruction/fail-close (2 chained triggers), and confirmed exponential blowup / real-API hang >5s on a 96-byte input (3 nested triggers; direct trace killed after minutes). B5-fix6 bounced to Next Steps as B5-fix7 with 6 RED tests (4 unit + 2 integration) + 8 green regression pins (sentinel-smuggling x5, security-battery x3). Status → qa-fail, current_role → developer, qa_cycles → 6.

## QA Notes archive (overflow from contract "QA Notes", kept last 5 there)
...
RESOLVED (R11, cycle 2): formula fixed to `hasExactDuplicate || propositionMissing`; confirmed matching in cycle-2 QA (see contract QA Notes cycle 2 and Completed/UI2 entry).
- 2026-07-26T02:14:19Z — Manager: QA final round accepted (commit clean; 6 RED verified). THIRD defect family reproduced: tail double-processing -> duplication/destruction/exponential blowup. Naive rawdata-clear fix REJECTED after probe (regresses RCDATA prose). Ruling R17: conditional salvage — manager-probed clean on all three families. Lock → developer; spawning fix7.
...
- 2026-07-26T07:45:34Z — Manager: QA cycle-7 accepted procedurally (commit clean, 4 RED verified) but the FINDING is OVERRULED per R18: manager cross-checked the tokenizer — `plaintail`/`<b` parse as img ATTRIBUTES, browser renders `Y Z`, byte-identical to our output. Same family as the accepted `<appendix A>` limitation, which QA was briefed to exclude. R17 mechanism itself verified clean by QA (0 duplication/destruction, <1ms). Planner-role agent to amend the 4 tests to pin browser-faithful behavior; then review.
- 2026-07-26T07:46:14Z — planner (R18 test amendment) → a40e0ab5bc7f298db (sonnet, high)
- 2026-07-26T07:50:24Z — Manager: R18 amendment verified (test files only; teeth kept — amended tests assert no live markup). FULL EVALUATOR manager-run: 238 backend + 60 frontend. All 12 items Completed. contract_lint PASS 146. Sprint → review, lock released. Open director decisions: length cap; raw+sanitized dual storage (future sprint).
