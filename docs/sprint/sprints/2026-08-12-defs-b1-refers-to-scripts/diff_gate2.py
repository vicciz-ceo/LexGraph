"""Gate-2 raw delta for sprint 2026-08-12-defs-b1-refers-to.

Diffs current/records.jsonl against baseline/records.jsonl using the SAME
key() tuple measure_actual_production.py's own compare() uses (jurisdiction,
source_file, source_row, term, definition_text, scope). Deliberately does
NOT call measure_actual_production.py --compare, because that mode enforces
a hard-coded EXPECTED_CHANGED_HASH from the prior M-R124 certified run --
there is no pre-existing certified ledger for this sprint's incremental
delta (P-R11: run, then adjudicate; no hand-authored ledger).
