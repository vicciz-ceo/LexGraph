...
- R17 Tail double-processing (manager, post-audit QA round): QA found a THIRD family, manager-reproduced and severe. `_sanitize_once` snapshots `parser.rawdata` BEFORE `parser.close()`, but `close()` itself flushes that same pending text through `handle_data` — so an unclosed RCDATA/CDATA wrapper containing a nested tag gets its tail counted TWICE. Effects: `'<iframe><b>x</b>'` -> `'x>x'` (authored text DUPLICATED and growing); two triggers -> `''` (document destroyed); three triggers -> exponential blowup (>5s on a 96-byte input; 2+ minute API hang). Rejected fix: clearing `rawdata` before `close()` kills the blowup but re-loses RCDATA tail prose (manager-probed: the `<Title>` sentence regressed). Required fix: make the salvage CONDITIONAL — record the collected-text length after `feed()`, call `close()`, and append `_salvage_trailing_prose(leftover)` ONLY when `close()` did not itself emit the tail (i.e. the text length did not grow). Manager-probed: triggers one/two/three -> `'x'`/`'xy'`/`'xyz'` in 0.0001s, 10/12 prose byte-exact (the 2 exceptions lose only the tag, the accepted limitation), 0 leaks across 14 attack shapes, 2000-chain 0.0040s, charref+16k 0.0001s, idempotent.
- R18 QA cycle-7 finding OVERRULED as the accepted limitation (manager, 2026-07-26): QA reported `'<img plaintail <b>Y</b> Z'` -> `'Y Z'` as a new G13 defect. Manager cross-checked with the tokenizer directly: `plaintail` and `<b` are parsed as ATTRIBUTES of `img`, so a browser renders exactly `Y Z` — our output is byte-identical to browser behavior. This is the SAME family as the already-accepted limitation (`see <appendix A> for details` -> `see  for details`): text between `<tagname` and the next `>` is dropped, whether that `>` is its own or a later tag's. QA's brief listed this family as explicitly not-a-finding; the report over-classified it as distinct. Rejected remedy: special-casing "borrowed `>`" would require departing from browser-faithful parsing with a heuristic on the security-critical path — precisely the whack-a-mole R12 banned, traded for a rare prose case. Disposition: the 4 RED tests assert out-of-scope behavior; a Planner-role agent amends them to pin the browser-faithful behavior as CORRECT, with the rationale in the test docstring. FUTURE-SPRINT RECOMMENDATION for the director: the durable remedy for authors typing `<` in prose is storing raw + sanitized text separately (schema change, out of scope under R8), not weakening the sanitizer.
