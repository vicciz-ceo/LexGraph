...
|---|---|---|---|---|
| R1 | **TX `2009.003` double-assertion hazard.** The section now yields 8 candidates: our correct combined N-term row, the 4 ORIGINAL degenerate 1-term rows (`;`, `""`), and 3 already-working entries. A real mention of e.g. "contested case" can draw **two** `USES_DEFINITION` assertions — one to the correct row, one to a degenerate row. | Developer verified live (`extract_definitions_from_section` → 8 candidates); persist-layer dedup key is `(article_id, sorted(terms))` so the rows do not collapse | **markers + this panel, JOINTLY TRACKED** (M-R25; entry-boundary damage per M-R5) | **AMENDED M-R25 — closes on PERSISTED/ASSERTED, not "produced".** The hazard is a DOUBLE ASSERTION; a candidate that never persists cannot draw one, so "produced" was the wrong altitude (my predecessor's shorthand, caught by the markers manager asking rather than inheriting it). Today both persist anyway — the degenerate row's terms tuple `('contested case',)` differs from F5's `('contested case','party','person','rule')`, so the `(article_id, sorted(terms))` key differs and neither absorbs the other. Two pins at two altitudes: markers pins the degenerate PRODUCTION at its own length-based class-B altitude; this panel pins the ASSERTION outcome (a mention draws exactly ONE `USES_DEFINITION`). Both close when core-follow-on-3's boundary discriminator lands |
| R3 | **`"Taken"` @ `STATE_OR_T41_C496_S496.716` — CROSS-PANEL HANDOFF, zero-miss critical.** The OR row's 5th term is defined with PLAIN `means`, not a cross-reference idiom. Under E3 it is family 1's mechanism and was never F6's to keep, so the F6 narrowing drops it. It is NOT abandoned: it is handed to `claude/defs-us-scoped-inline`. | Developer proved live: 4 of 5 terms captured post-narrow; the 5th fails only because `means` is family 1's | **scoped-inline** (accepting), named here until proven | **ONLY** when scoped-inline's family-1 live path PROVABLY captures this exact row — a live run or a named-row test on their side, confirmed by their manager. **A general claim that their family "covers the shape" does NOT close this** (the program has been burned by that inference twice). If they veto ownership, escalate to the program manager. |
| R4 | **Same-term multi-occurrence: keep-first can retain the WRONG sentence.** When one term is introduced by several appositions in one body, M-R14's dedup keeps the FIRST occurrence's `definition_text`. That first occurrence can be an incidental re-mention rather than the genuine introduction. **Verified case** `STATE_DE_T18_C35_SIV_S3578`: the kept first `"ASAM"` sits in the sentence defining `"ASAM criteria"` (a DIFFERENT term), while the DROPPED second occurrence is the real acronym introduction (`…American Society of Addiction Medicine ("ASAM")`). Pre-fix the same loss happened NONDETERMINISTICALLY at the persistence race; it is now deterministic at extraction — a strict improvement in predictability, not in correctness. | **Manager-measured on the real corpus** (79,500 rows sampled, same seed as the fire-rate runs): 19 multi-occurrence terms; **17 (89.5%) have materially different sentence texts**; 15 rows affected = **0.019% of sampled rows** (~385 rows extrapolated to the 2,038,247-row corpus). Examples: DC `50-1401.01`/`BOP` (3 occ, 2 distinct), DC `7-2051`/`OSSE`, DE `3578`/`ASAM`, LA `9038.76`/`district exclusions` | this sprint (named, not fixed) | a fix lands that selects by a **measured occurrence-quality signal** (e.g. presence of an expanded form immediately preceding the parenthetical), NOT a position heuristic. D-CERT's classification will surface instances. The Developer correctly DECLINED to invent a keep-best heuristic rather than guess. |
| R5 | **Cross-path duplicate — a class no per-primitive dedup can reach.** The same term is emitted once by `_apposition_candidates` and once by `_cross_reference_candidates`; each primitive is individually clean, so M-R14's and M-R17's `seen_terms` guards are both structurally blind to it (the union happens in `_extract_ordinary_body`). | Manager-measured, then CORRECTED by QA cycle 2: **2 rows, not 1** — `HI STATE_HI_D4_T36_C667_S667-101` (`association`) and `DE STATE_DE_T12_C9_S902` (`the Code`). My 79,500-row stride sample found only the first; QA's full-corpus run found the second. Both collapse to one row at the persist layer, verified live. | **this sprint (named, not fixed)** | a dedup at the `_extract_ordinary_body` union altitude lands, with a Planner RED first. Severity is low (harmless at persist), so it was NOT bolted onto a running Developer's brief |
| R6 | **Baseline cannot open a block at a hyphen-suffixed marker.** `us_profile._MARKER_TOKEN_RE = \(\w+\)` and `\w` excludes `-`, so `(9-a)` never starts an entry: the whole entry is swallowed into the preceding block. Cross-reference-idiom entries are rescued by F6 (which is how M-R23 became visible); **plain-`means` entries are MISSED outright.** | Root cause confirmed at source by two panels independently. Shape frequency (NOT a miss count): 841 occurrences — TX 817, NY 19-20, NH 4, DC 1. My original "TX-only" framing was WRONG and is corrected in M-R27 addendum — a negative result on ONE sub-shape across 8 states, over-generalised to the whole class | **markers** (family 3, entry-marker mismatch) — accepted by their manager, who is re-deriving in their own population before quoting any number | markers' entry-marker work covers hyphen-suffixed markers |
| Q-A | **F5's `_IDIOM_RE` never matched "have/has the meaning(s)".** Real 4-term clause `USC_T2_C20_S900` dropped entirely. Same class as cycle-1 finding 3, which fixed the analogous gap in sibling module F6; F5 never got it. | QA cycle 2, full-corpus sized: **70 missed terms** (federal 57, il 7, hi 2, mt 2, mi 1, nd 1). RED committed by QA | **this sprint — IN FAMILY, being fixed** | the Developer pass now in flight lands and QA's RED goes green |
| Q-D | **NY literal-`\n` "total loss" — WITHDRAWN as a probe artifact.** Reported as 1,470 Definitions sections yielding zero candidates; the measurement called extraction on RAW parquet text, bypassing the ingest-time fix (`ingest_us_statutes.py:237`). I independently "verified" it and made the same error. | Production-faithful re-measure, mine: raw 1,479/1,479 zero (100%) vs post-ingest 1,479/1,245 zero (84.2%). Cross-panel timeline now closed: pre-devC 84.2% (this tree) / 79.9% (markers' tree) -> **post-devC 160 = 10.8%, the current citable figure** | **markers** (already-known NY residual; their merged devC widening moves NY captured 298 -> 1,319) | already substantially closed on their branch |
