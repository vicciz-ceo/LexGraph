"""Item 8 (QA-fail cycle 1 re-certification, sprint 2026-08-23-defs-debt-31)
100% anchor-granularity delta adjudication -- a straight copy of Items 1-4's
own `adjudicate_gate5_delta.py` (unchanged logic/categories, see that file
for the full account) pointed at a NEW `gate5-adjudication-v2/` output dir
so the original QA-bounced run's own evidence stays intact for comparison.
Reads `current/records.jsonl` (re-generated fresh against Items 6-7's own
fixes) and `baseline/records.jsonl` (REUSED unchanged -- `main`@8850401 has
not moved since the original run), and classifies every changed record per
the known trap: "Decompose removals at ANCHOR granularity before reacting
...
OUT_DIR = SCRIPTS_DIR / "gate5-adjudication-v2"
