...
A4 -- US-NY's DOMINANT zero-yield shape (family collapse, item A3).

**Measured, not hypothesised.** US-NY's zero-yield Definitions-headed
...
splitter cannot anchor on) -- US-NY is simply not yet in that rule's
...
EVERY real US-NY zero-yield row, corpus-wide, read-only, nothing written
to the tree) rescues **82.9% (1,046/1,262, post-ingest)** of US-NY's zero-yield population -- this
is the single largest lever for US-NY, not a new convention.

Real row `STATE_NY_AACA_TP_A40_S40.03` demonstrates it cleanly. `extract_quote_anchored_entries`
...
false positive) -- US-NY rows that cite adjacent statute sections
...
RED today: US-NY has ZERO family-3 rules registered (confirmed by grep
of `backend/app/definition_links/rules/` before writing this fixture) --
`STATE_NY_AACA_TP_A40_S40.03` yields 0 definitions via today's real pipeline even though its
...
FIXTURE_PATH = (
    Path(__file__).resolve().parents[1] / "fixtures" / "us_statutes" / "us_markers_ext_a_quoteengine_ny_rows.json"
)
...
def test_fixture_row_is_the_expected_real_ny_row():
    ...
    row = _load_row()
    assert row["act_id"] == "STATE_NY_AACA_TP_A40_S40.03"


def test_real_pipeline_recovers_ny_quote_anchored_definitions(db_session, matter_with_users):
    ...
    not a stub). Today US-NY has no registered `EntrySplitterRule`/
    `TermClauseRule` at all, so this creates 0 `Definition` rows; the
    fix (per A3) is registering US-NY into the ALREADY-BUILT
    `us_markers_inline_quote.py` engine (or a thin US-NY-scoped sibling
    ...
    ingest_us_statute_rows(
        ...
        title="STATE_NY_AACA_TP_A40_S40.03 A4 quote-engine RED",
        rows=[row],
        jurisdiction="US-NY",
    )
    ...
    expected_subset = ['Trust', 'Board', 'Board of regents']
    assert set(expected_subset) <= set(by_term), (
        f"expected at least {expected_subset!r} to be captured from US-NY's real "
        f"quote-anchored Definitions body; today's real pipeline yields {sorted(by_term)!r} "
        "-- US-NY has zero family-3 rules registered (the A3 family-collapse fix, "
        "not a new convention)."
    )
    spot = by_term['Board of regents']
    assert spot.definition_text.strip() == 'the board of regents of the\nuniversity of the state of New York.', f"got {spot.definition_text!r}"
