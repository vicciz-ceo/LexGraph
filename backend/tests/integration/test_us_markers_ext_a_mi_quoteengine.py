...
A4 -- US-MI's DOMINANT zero-yield shape (family collapse, item A3).

**Measured, not hypothesised.** US-MI's zero-yield Definitions-headed
...
splitter cannot anchor on) -- US-MI is simply not yet in that rule's
...
EVERY real US-MI zero-yield row, corpus-wide, read-only, nothing written
to the tree) rescues **84.9% (948/1,116)** of US-MI's zero-yield population -- this
is the single largest lever for US-MI, not a new convention.

Real row `STATE_MI_C333_AAct-368-of-1978_S333.20169` demonstrates it cleanly. `extract_quote_anchored_entries`
...
false positive) -- US-MI rows that cite adjacent statute sections
...
RED today: US-MI has ZERO family-3 rules registered (confirmed by grep
of `backend/app/definition_links/rules/` before writing this fixture) --
`STATE_MI_C333_AAct-368-of-1978_S333.20169` yields 0 definitions via today's real pipeline even though its
...
FIXTURE_PATH = (
    Path(__file__).resolve().parents[1] / "fixtures" / "us_statutes" / "us_markers_ext_a_quoteengine_mi_rows.json"
)
...
def test_fixture_row_is_the_expected_real_mi_row():
    """Sanity: byte-verified against the real corpus row."""
    row = _load_row()
    assert row["act_id"] == "STATE_MI_C333_AAct-368-of-1978_S333.20169"


def test_real_pipeline_recovers_mi_quote_anchored_definitions(db_session, matter_with_users):
    ...
    not a stub). Today US-MI has no registered `EntrySplitterRule`/
    `TermClauseRule` at all, so this creates 0 `Definition` rows; the
    fix (per A3) is registering US-MI into the ALREADY-BUILT
    `us_markers_inline_quote.py` engine (or a thin US-MI-scoped sibling
    ...
    ingest_us_statute_rows(
        ...
        title="STATE_MI_C333_AAct-368-of-1978_S333.20169 A4 quote-engine RED",
        rows=[row],
        jurisdiction="US-MI",
    )
    ...
    assert set(expected_subset) <= set(by_term), (
        f"expected at least {expected_subset!r} to be captured from US-MI's real "
        f"quote-anchored Definitions body; today's real pipeline yields {sorted(by_term)!r} "
        "-- US-MI has zero family-3 rules registered (the A3 family-collapse fix, "
        "not a new convention)."
    )
    spot = by_term['HIV']
    assert spot.definition_text.strip() == 'human immunodeficiency virus.', f"got {spot.definition_text!r}"
