...
Developer C's US-ND registration is this file's biggest population by far
(49 of the sprint's 75 total extra terms are ND, all newly captured where
baseline previously found none, per `## M34`). RULING U-R16 vacated the
earlier spurious-fragment theory -- every extra term here is real. 7 of the
49 (across this file's 3 rows) are class-B (`## M37`'s closed 15-of-75
list) plus one more this Planner independently found; all pinned for
PRESENCE only in this file, never text -- their defective captures are
...
def test_c5_guard_state_nd_t57_c57_39_2_s57_39_2_01(db_session, matter_with_users):
    ...
    # Re-pinned per U-R16/U-R17 (M34/M35/M37): devC's US-ND registration adds
    # 20 genuine new terms, verified against source (U-R16 vacated the
    # spurious theory). 4 of the 20 are class-B (`## M37`'s closed list:
    # 'Bundled transaction', 'Farm machinery repair parts', 'Gross
    # receipts', 'sale at retail') -- pinned for PRESENCE here only; their
    # defective text is pinned separately, RED, in
    # `test_us_markers_c5guard_class_b_boundary_defects.py`, not baked in.
    assert sorted(by_term) == ['Bundled transaction', 'Certified automated system', 'Certified service provider', 'Commissioner', 'De minimis', 'Delivery charges', 'Direct mail', 'Drug', 'Farm machinery', 'Farm machinery repair parts', 'Gross receipts', 'Irrigation equipment repair parts', 'Lease or rental', 'Local governmental unit', 'Prescription', 'Relief agency', 'Sale', 'Sales tax', 'Tangible personal property', 'attachment unit', 'sale at retail'], f"got {sorted(by_term)!r}"
...
def test_c5_guard_state_nd_t57_c57_02_s57_02_01(db_session, matter_with_users):
    ...
    # Re-pinned per U-R16/U-R17 (M34/M35/M37): devC's US-ND registration adds
    # 16 genuine new terms, verified against source. 4 are class-B (`## M37`'s
    # closed list: 'Agricultural property', 'Air carrier transportation
    # property', 'Centrally assessed property', 'Commercial property') plus
    # ONE this Planner independently found beyond the closed list
    # ('Nonprimary residential property', reported per M37's own
    # invitation) -- all pinned for PRESENCE here only; their defective text
    # is pinned separately, RED, in
    # `test_us_markers_c5guard_class_b_boundary_defects.py`, not baked in.
    assert sorted(by_term) == ['Agricultural property', 'Air carrier transportation property', 'Assessed valuation', 'Centrally assessed property', 'Commercial property', 'Credits', 'Governing body', 'Nonprimary residential property', 'Primary residential property', 'Railroad property', 'Raising agricultural crops', 'Residential property', 'True and full value', 'Unencumbered cash', 'moneys', 'piece or parcel of land', 'taxing district'], f"got {sorted(by_term)!r}"
...
def test_c5_guard_state_nd_t51_c51_19_s51_19_02(db_session, matter_with_users):
    ...
    # Re-pinned per U-R16/U-R17 (M34/M35/M37): devC's US-ND registration adds
    # 13 genuine new terms, verified against source ('Offer' itself was
    # already an existing baseline pin, confirmed NOT one of the extras --
    # M37's withdrawal, cross-checked against this Planner's own M26
    # inventory). 3 of the 13 are class-B (`## M37`'s closed list:
    # 'Commissioner', 'Franchise', 'Rule') -- pinned for PRESENCE here only;
    # their defective text is pinned separately, RED, in
    # `test_us_markers_c5guard_class_b_boundary_defects.py`, not baked in.
    assert sorted(by_term) == ['Advertisement', 'Area franchise', 'Commissioner', 'Franchise', 'Franchise fee', 'Franchisee', 'Franchisor', 'Offer', 'Offer to purchase', 'Order', 'Person', 'Publish', 'Rule', 'State', 'Subfranchisor'], f"got {sorted(by_term)!r}"
