...
Developer C's US-NY registration surfaces genuine new terms on 2 of this
file's rows, verified against source (U-R16 vacated the spurious theory).
NY has NO class-B terms (`## M37`'s closed 15-of-75 list has none from NY):
'retailer' was investigated and accepted-cleared as a "cosmetic residue
class" (trailing `"; and"` list connector, no content lost, not truncation)
-- named so it is not rediscovered as a defect."""
...
def test_c5_guard_state_ny_aenv_a27_t18_s27_1803(db_session, matter_with_users):
    ...
    # Re-pinned per U-R16/U-R17 (M34/M35/M37): devC's US-NY registration adds
    # 6 genuine new terms, verified against source. None are class-B --
    # 'retailer' was investigated and accepted-cleared (trailing "; and"
    # list connector, this corpus's drafting convention, no content lost).
    assert sorted(by_term) == ['battery manufacturer', 'consumer', 'food store', 'place of business', 'rechargeable battery', 'retailer', 'sale'], f"got {sorted(by_term)!r}"
...
def test_c5_guard_state_ny_aisc_a71_s7101(db_session, matter_with_users):
    ...
    # Re-pinned per U-R16/U-R17 (M34/M35/M37): devC's US-NY registration adds
    # one genuine new term ('acquire the assets of'), verified against
    # source, not class-B.
    assert sorted(by_term) == ['Acquisition of assets', 'Company', 'Consolidated company', 'Consolidation', 'Constituent company', 'Merger', 'Surviving company', 'acquire the assets of'], f"got {sorted(by_term)!r}"
